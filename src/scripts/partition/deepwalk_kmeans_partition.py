...
INPUT_FILE = "data/vectors/deepwalk/deepwalk_embeddings.pickle"
OUTPUT_DIR = "data/partitions/deepwalk_kmeans/"
...
def load_graph(input_file):
    ...
    logger.info(f"Loading DeepWalk embeddings from {input_file}")
    try:
        with open(input_file, 'rb') as f:
            data = pickle.load(f)
        return data
    except Exception as e:
        logger.error(f"Error loading DeepWalk embeddings: {e}")
        raise
...
def visualize_clusters(G, cluster_labels, node_pos=None, output_file=None):
    ...
    plt.title(f"Graph Clustering with DeepWalk+KMeans (k={num_clusters})")
...
def main():
    """Main function to execute the DeepWalk+KMeans partitioning workflow."""
    ...
    logger.info(f"Loaded DeepWalk embeddings for {len(data)} nodes")
    ...
    metrics = {
        'algorithm': 'deepwalk-kmeans',  # Changed from 'kmeans' to 'deepwalk-kmeans'
    ...
    logger.info("DeepWalk+KMeans partitioning completed successfully")
